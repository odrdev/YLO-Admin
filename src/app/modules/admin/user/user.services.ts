...
import { iUser, iUserList, iUserResult } from './user.type';
@Injectable({providedIn: 'root'})
export class UserService
{
    // Private

    private _item: BehaviorSubject<iUser | null> = new BehaviorSubject(null);
    private _List: BehaviorSubject<iUser[] | null> = new BehaviorSubject(null);
    private _pagedList: BehaviorSubject<iUserList[] | null> = new BehaviorSubject(null);
     ...
     * Getter for services
     */
    get PagedList$(): Observable<iUserList[]>
    {
        return this._pagedList.asObservable();
    }
    get List$(): Observable<iUser[]>
    {
        return this._List.asObservable();
    }
    get item$(): Observable<iUser>
    {
    ...
    getList(): Observable<iUser[]>
    {
        ...
        return this._httpClient.get<iUser[]>(this.apiURL,this.getHeaders()).pipe(
            tap((res) =>
            {
...
/**
     ...
     * @param txtsearch
     */
    getListPaging(txtsearch = "", page = 0, size = 10,sortField="id", sortAscending='asc'): Observable<iUserResult>
    {
        ...
        console.log(param)
        return this._httpClient.post<iUserResult>(url,param,this.getHeaders()).pipe(
            tap((res) =>
            {
    ...
    create(item:iUser): Observable<iUser>
    {
        return this.PagedList$.pipe(
            take(1),
            switchMap(products => this._httpClient.post<iUser>(this.apiURL, item, this.getHeaders()).pipe(
                map((newItem) =>
                {
     ...
     * @param item
     */
    update(id: any, item: iUser): Observable<iUser>
    {
        console.log(item)
        return this.PagedList$.pipe(
            take(1),
            switchMap(items => this._httpClient.put<iUser>(this.apiURL +  id, item, this.getHeaders()).pipe(
                map((updatedItem) =>
                {
